...
vertmeas.species1 <- read.csv("PhyloPreppedVertMeas.csv") %>% 
  column_to_rownames(var = colnames(vertmeas.species1)[1])
...
```
CBL and body shape VERY significant here.
...
```{r}
pgls_d_Habitat <- gls(d_mean ~ Habitat_Friedman, correlation = corBrownian(phy = verttree),
                      data = vertmeas.species1, method = "ML")
...
```{r}
pgls_CBL_Habitat <- gls(CBL_mean ~ Habitat_Friedman, correlation = corBrownian(phy = verttree),
                        data = vertmeas.species1, method = "ML")
...
```{r}
h1 <- ggplot(vertmeas.species1, aes(x=Habitat_Friedman, y=d_mean, color=Habitat_Friedman)) +
  geom_point(alpha = 0.5, position = position_jitter(width=0.2)) +
  stat_summary(fun.data = mean_sdl)


h2 <- ggplot(vertmeas.species1, aes(x=Habitat_Friedman, y=CBL_mean, color=Habitat_Friedman)) +
  geom_point(alpha = 0.5, position = position_jitter(width=0.2)) + 
  stat_summary(fun.data = mean_sdl)
...
*COME BACK HERE, HAVE TO FIX REMOVING THE OUTLIER, BECAUSE GENUS IS NO LONGER A COLUMN NAME*
```{r}
vertmeas.species1 <- names(vertmeas.species1)[1] = "Genus"
...
```{r}
pglsedit_d_Habitat <- gls(d_mean ~ Habitat_Friedman, correlation = corBrownian(phy = verttreeedit),
...
```{r}
pglsedit_CBL_Habitat <- gls(CBL_mean ~ Habitat_Friedman, correlation = corBrownian(phy = verttreeedit),
                        data = pglsedit, method = "ML")
...
```{r}
ha1 <- ggplot(pglsedit, aes(x=Habitat_Friedman, y=d_mean, color=Habitat_Friedman)) +
  geom_point(alpha = 0.5, position = position_jitter(width=0.2)) +
  stat_summary(fun.data = mean_sdl)


ha2 <- ggplot(pglsedit, aes(x=Habitat_Friedman, y=CBL_mean, color=Habitat_Friedman)) +
  geom_point(alpha = 0.5, position = position_jitter(width=0.2)) + 
  stat_summary(fun.data = mean_sdl)
...
```
Why is it not significant here -- do I have to rerun renv? --> even with the changes it's still 0.4104...why is it not significant anymore??
