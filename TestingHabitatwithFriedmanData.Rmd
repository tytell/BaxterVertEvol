...
```
Tried to rerun with renv, still not working.

#Mean PGLS with D_Pos and habitat consensus

```{r}
pgls_DPos_HabitatConsensus <- gls(D_Pos ~ Habitat, correlation = corBrownian(phy = verttree),
                      data = vertmeas.species1, method = "ML")
...
```{r}
pgls_alphaPos_HabitatConsensus <- gls(alpha_Pos ~ Habitat, correlation = corBrownian(phy = verttree),
                      data = vertmeas.species1, method = "ML")
summary(pgls_alphaPos_HabitatConsensus)
anova(pgls_alphaPos_HabitatConsensus)
```
But then why is alphaPos still significant here but d is not?

```{r}
pairs(emmeans(pgls_alphaPos_HabitatConsensus, "Habitat"))
```
Why is just demersal-pelagic significant here?
...
```{r}
pglsedit_d_HabitatConsensus <- gls(d ~ Habitat, correlation = corBrownian(phy = verttreeedit),
                      data = pglsedit, method = "ML")
summary(pglsedit_d_HabitatConsensus)
anova(pglsedit_d_HabitatConsensus)
```
Why is this still not significant?
...
```{r}
pglsedit_alphaPos_HabitatConsensus <- gls(alpha_Pos ~ Habitat, correlation = corBrownian(phy = verttreeedit),
                      data = pglsedit, method = "ML")
...
```{r}
pglsedit_CBL_HabitatConsensus <- gls(CBL ~ Habitat, correlation = corBrownian(phy = verttreeedit), data = pglsedit, method = "ML")
