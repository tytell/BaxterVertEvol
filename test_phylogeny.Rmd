...
Check correlation between notochordal foramen diameter and habitat
```{r}
plot(vertmeas, c("d", "Habitat"))
```
...
```{r}
d_PICs <- vertmeas[,"d"]
habitat_PICs <- vertmeas[,"Habitat"]

names(d_PICs) <- names(habitat_PICs) <- rownames(vertmeas)

dPIC <- pic(d_PICs, verttree)
habPIC <- pic(habitat_PICs, verttree)

d_habPIC_model <- lm(dPic ~ habPIC - 1)

summary(d_habPIC_model)
```
