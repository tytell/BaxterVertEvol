...
#Add Column to vertmeasflexjoin with Max
```{r}
vertmeasflexjoinmax <- vertmeasflexjoin %>%
  group_by(Species) %>%
  select(En, Qn, Gn, Vn) %>%
...
```{r}
vertmeasflexjoinmin <- vertmeasflexjoin %>%
  group_by(Species) %>%
  select(En, Qn, Gn, Vn) %>%
...
vertmeasflexjoinmin
```
Why are these the exact same? I'm really confused...
