...
```{r}
left_join(fishbaseprelim,fishbasedatamergeddepth, by = "Species")
```


```{r}
vertmeas_mean <- 
  mutate(vertmeas, CBL_mean = vertmeas %>% group_by(Species) %>% summarize(mean(CBL)), d_mean = vertmeas %>% group_by(Species) %>% summarize(mean(d)), alpha_Pos_mean = vertmeas %>% group_by(Species) %>% summarize(mean(alpha_Pos)), D_Pos_mean = vertmeas %>% group_by(Species) %>% summarize(mean(D_Pos)), alpha_Ant_mean = vertmeas %>% group_by(Species) %>% summarize(mean(alpha_Ant)), D_Ant_mean = vertmeas %>% group_by(Species) %>% summarize(mean(D_Ant)))
