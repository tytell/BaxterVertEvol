...
```{r}
here(i_am("Code/CheckSpeciesMatch.Rmd"))
```

## Vertebral measurements

```{r}
fullvertmeas <- read_csv(here('Raw Data', "MasterVert_Measurements.csv")) %>%
...
And write it out!
```{r}
write_csv(fullvertmeas_matched, here('Processed Data', "MasterVert_Measurements_Matched.csv"))
```
...
```{r}
fullvertmeas <- read_csv(here('Processed Data', "MasterVert_Measurements_Matched.csv")) %>%
  separate(MatchSpecies, into=c("MatchGenus", "MatchSpecies"), sep="_") %>%
