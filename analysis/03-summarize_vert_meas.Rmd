...
```{r}
vertdata |> 
  filter(Habitat_Friedman != Habitat) |> 
  distinct(Species, .keep_all = TRUE) |> 
  select(Species, starts_with('Habitat')) |> 
  arrange(Habitat_Friedman)
```

```{r}
vertdata |>
  unite(MatchSpecies, MatchGenus, MatchSpecies) |> 
  filter(Indiv == 1) |> 
  group_by(MatchSpecies, Pos) |> 
  filter(n() > 1)
...
vertdata <-
  vertdata |> 
  filter(Indiv == 1 & Species != 'Xiphister_atropurpureus')
```
...
vertdata_summary <-
  vertdata2 %>%
  dplyr::filter(Pos != 0.2 & Pos != 0.3) %>%
  group_by(Species, Indiv, Habitat, Water_Type, MatchSpecies, MatchGenus, 
           alltaxon, Order, Family) %>%
  dplyr::summarize(fineness = mean(fineness),
                   across(c(CBL, d, alphaAnt, alphaPos, DAnt, DPos,
