...
vertdatapts <-
  vertdata |> left_join(select(vertdata_sp, Species, Tip, FullName), 
                      by = "Species") |> 
  filter(Indiv == 1) |> 
  select(Habitat, Pos, starts_with('Pt'), fineness, Tip, FullName) |> 
...
```{r}
manova.sum <-
  summary(manova.pgls, test = 'Pillai')
manova.sum
```

```{r}
manovatabsall <-
  manova.sum$stats.table |> 
  as.data.frame() |> 
  rownames_to_column(var = 'Effect') |> 
  mutate(var = 'Overall means') |> 
...
```{r}
dyl = 0
yl = max(vertdata_sp$CBL_mn) + dyl

p1 <- anovatabs |> 
  filter(var == 'CBL_mn' & Effect == 'Habitat') |> 
  pull(p)

CBL_mn_panel <- ggplot(vertdata_sp, aes(x = Habitat, y = CBL_mn, color = Habitat, shape = Habitat)) +
  geom_quasirandom(width=0.3, alpha = 0.5) +
  stat_summary(fun.data = 'mean_cl_boot') +
  # stat_summary(aes(group = 1), fun = 'mean', geom='line', color='black') +
  # stat_pvalue_manual(data = filter(pairtests, var == 'CBL_mn',
  #                                  Z > 0.5),
  #                    y.position = yl, step.increase = 0.12,
