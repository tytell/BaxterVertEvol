...
This is the whole tree from the Fish Tree of Life: https://fishtreeoflife.org/.
```{r}
tree <- read.tree(here('data', 'actinopt_12k_treePL.tre.xz'))
```

```{r}
tax <- read_csv(here('data', 'PFC_taxonomy.csv.xz')) |> 
...
```{r}
allspecies <- 
  allspecies %>% separate(FullName, sep='_', into=c('Genus', 'Species'), 
                          extra='drop', remove=FALSE)
```
...
ourspecies <-
  ourspecies |> 
  left_join(tax, by = c("Genus", "Species"))
```
...
For species that aren't in the tree, we have to find the most appropriate other species. First, find species in our data set that are not in the tree.
```{r}
missingspecies <- anti_join(ourspecies, allspecies, by=c("Genus", "Species"))
missingspecies
```

Look for species in the tree in the same genera as the missing species:
```{r}
left_join(missingspecies, allspecies, by=c("Genus")) %>%
  group_by(Genus, Species.x) %>%
  dplyr::summarize(n = sum(!is.na(Species.y)))
```

For species and genera with 1 or more matches in the tree, we can assume that the phylogenetic relationship is the same as another species in the genus, so long as our data set doesn't have more than one species in that genus.
...
For species where the genus only shows up once in our data set, we can choose any other species in the tree that is in the same genus
```{r}
matchspecies <-
  missingspecies %>%
  ...
  group_by(Genus, Species.x) %>%
  dplyr::summarize(MatchSpecies = first(Species.y),
...
Here is the full list of our species and the best matching species from the tree.
```{r}
matchspecies <-
  ourspecies %>%
  ...
  select(Genus, Species, MatchGenus, MatchSpecies, alltaxon:Family) %>%
